...
tape('Check that server responds with correct index.html', function(t) {
    shot.inject(server.router, {
        ...
        t.ok(response.payload.indexOf("<!DOCTYPE html>") > -1,
            'server says oi heres there html');
        t.end();
    });
});
...
tape('check that css load correctly', function(t) {
    shot.inject(server.router, {
        ...
        fs.readFile(__dirname + '/../' + 'style.css', function(
            err, res) {
            t.equal(shotResponse.payload.toString(
                    'utf8'), res.toString('utf8'),
                'should load css properly');
            t.end();
        });
    });
});
...
tape('check that unknown resource request returns 404', function(t) {
    t.plan(3);
    ['unknown.html', 'unknown.css', 'unknown.js'].forEach(function(
        fileName) {
        shot.inject(server.router, {
            method: 'GET',
            url: '/' + fileName
